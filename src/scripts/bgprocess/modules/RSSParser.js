...
define(["he"], function (he) {
    class RSSParser {
        getLink() {
            const urlMatcher = /.+:\/\//;
            const base = urlMatcher.exec(this.source.get("base"))
                ? this.source.get("base")
                : this.source.get("url"); // some feeds only give relative URLs but no base
        ...
        getSourceTitle() {
            ...
            if (!title || !title.textContent.trim()) {
                title = this.document.querySelector(
                    "channel > description, feed > description, rss > description"
                );
            }
        ...
        getArticleTitle() {
            ...
            const title = node
                ? this.currentNode.querySelector("title").textContent.trim()
                : "<no title>";
        ...
        getEnclosure(enclosureNode, title) {
            const enclosure = {};
            enclosure.url = enclosureNode.hasAttribute("url")
                ? enclosureNode.getAttribute("url").replace(/^(javascript:\.)/, "")
                : "";
            enclosure.name = he.decode(
                enclosureNode.hasAttribute("url")
                    ? enclosure.url.substring(enclosure.url.lastIndexOf("/") + 1)
                    : title
            );
            enclosure.type = enclosureNode.hasAttribute("type")
                ? enclosureNode.getAttribute("type")
                : "";
            enclosure.medium = enclosureNode.hasAttribute("medium")
                ? enclosureNode.getAttribute("medium")
                : this.getMediumFromType(enclosure.type, enclosure.name);
            enclosure.medium = enclosure.url.includes("youtube.com") ? "youtube" : enclosure.medium;
            enclosure.length = enclosureNode.hasAttribute("length")
        ...
        getEnclosures() {
            const node = this.currentNode;
            const knownUrls = [];
            const mediaTitleNode = [
                ...node.getElementsByTagNameNS("http://search.yahoo.com/mrss/", "title"),
            ][0];
            ...
            const enclosureNodes = [
                ...node.getElementsByTagNameNS("http://search.yahoo.com/mrss/", "content"),
            ];
        ...
        parse() {
            ...
            if (
                title &&
                (this.source.get("title") === this.source.get("url") || !this.source.get("title"))
            ) {
